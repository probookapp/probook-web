...
const COUPON_EXHAUSTED = "COUPON_EXHAUSTED";
...
export const POST = withSuperAdmin(async (req: NextRequest, ctx) => {
  ...
  // Calculate price based on billing cycle. Coupon redemption happens inside
  // the transaction below so a failed approval never consumes a use.
  const basePrice = request.billingCycle === "monthly" ? monthlyPrice : yearlyPrice;
  let price = basePrice;
  ...
  try {
    result = await prisma.$transaction(async (tx) => {
      // Redeem the coupon atomically inside the transaction: the conditional
      // updateMany guard prevents concurrent approvals from over-redeeming.
      if (request.couponCode) {
        ...
        if (coupon && coupon.isActive && (!coupon.expiresAt || coupon.expiresAt > new Date()) && (coupon.maxUses === null || coupon.currentUses < coupon.maxUses)) {
          ...
          if (planAllowed) {
            // Increment coupon uses only while uses remain
            const redeemed = await tx.coupon.updateMany({
              where: {
                id: coupon.id,
                ...(coupon.maxUses !== null ? { currentUses: { lt: coupon.maxUses } } : {}),
              },
    ...
    if (error instanceof Error && error.message === COUPON_EXHAUSTED) {
      return NextResponse.json({ error: "Coupon exhausted" }, { status: 409 });
    }
    throw error;
  }
