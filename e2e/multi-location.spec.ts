 ...
 * The default location is created lazily by the stock engine the first
 * time a stock change happens — so we seed a stocked product via the API before
 * exercising the locations UI, which guarantees the default one exists. It is
 * named after the business, so `company` from signup is its label.
 */
...
test.describe("Multi-location", () => {
  // The default location the stock engine creates is named after the business.
  let company = "";

  test.beforeEach(async ({ page }) => {
    ({ company } = await signUpSubscribed(page));
  });

  test("locations CRUD via the Locations page (create, edit, blocked default delete)", async ({
    page,
  }) => {
    // Seed a stocked product so the lazily-created default location exists.
    ...
    await expect(page.getByRole("row", { name: company })).toBeVisible();
    ...
    const mainRow = page.getByRole("row", { name: company });
  ...
  test("stock transfer default → Warehouse moves quantities, aggregate unchanged", async ({
    page,
  }) => {
    // Seed a product with 100 on hand (creates the default location + its level).
    ...
    await dialog.getByLabel("From").selectOption({ label: company });
    ...
    const main = levels.find((l) => l.location_name === company);
  ...
  test("adjust-stock targeting a location writes a per-location level + ledger row", async ({
    ...
    // ── Per-location level: Warehouse = 30, default untouched at 100 ─────
    const levels = await stockLevels(page, product.id as string);
    expect(levels.find((l) => l.location_name === "Warehouse")?.quantity).toBe(30);
    expect(levels.find((l) => l.location_name === company)?.quantity).toBe(100);
  ...
  test("product row shows a per-location stock breakdown with 2+ locations", async ({
    ...
    // Move some stock so both locations hold on-hand (default 60 / Warehouse 40).
    ...
    const levels = await stockLevels(page, product.id as string);
    expect(levels.find((l) => l.location_name === company)?.quantity).toBe(60);
